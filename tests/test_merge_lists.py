...
def test_merge_list_to_sort():
    with open('fixtures/list_keys_1.txt', 'r', encoding='utf-8') as keys_list:
        list_keys = ast.literal_eval(keys_list.read().strip())
    with (open('fixtures/list_of_paths_1.txt', 'r', encoding='utf-8')
          as paths_list):
        list_of_paths = ast.literal_eval(paths_list.read().strip())
    with (open('fixtures/list_values_1.txt', 'r', encoding='utf-8')
          as values_list):
        list_values = ast.literal_eval(values_list.read().strip())
    with (open('fixtures/list_merged_to_sort.txt', 'r', encoding='utf-8')
          as merged_list_to_sort):
